# ClinicalInterviewIR

## Hello Fellow Group Members, Im Looking Forward To Working With You All

### Ive created this repo in an attempt to centralize the work for the project, and easier for us all.

#### The goal is that individual components are easily accessible and individual contributions are recorded (although lets hope thats not necessary). Please message me in the groupchat and I can get you any permission you want. 

#### A few notes, this is a public repo, so please dont add any private keys (feel free to add to gitignore as you see fit) or hardcode anything. Additionally, as for code design, (im not the best so i cant judge) but please try to keep it well documented, ideally also using the typing module for python code. If there are more extensive notes you want to add, ive added a folder dedicated to contributions.




### Current Progress:
Front End   (HTML, JS):  Started <br>
Back End      (Python):  Created <br>
Pyannote Code (Python):  Created <br>
.....
